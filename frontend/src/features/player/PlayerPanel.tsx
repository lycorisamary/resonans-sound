        ...
        <Stack direction="row" spacing={1.25} alignItems="center">
          ...
          <Box sx={{ minWidth: 0, flex: 1 }}>
            ...
            <Typography variant="body2" color="text.secondary" noWrap>
              {track ? `${track.artist?.display_name || track.user?.display_name || track.user?.username || 'Unknown artist'} · ${formatTime(player.playerCurrentTime)} / ${formatTime(player.playerDuration || (track.duration_seconds ?? 0))}` : 'Playback stays active across pages.'}
            </Typography>
          </Box>

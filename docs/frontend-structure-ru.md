...
- `/` — главная: studio form, player, catalog
- `/login` — auth panel
...
`getPlayableQualityCandidates()`. В карточке также есть нативный `<audio controls>`
с прямым `/stream`, чтобы прослушивание не зависело только от кастомной кнопки
плеера.

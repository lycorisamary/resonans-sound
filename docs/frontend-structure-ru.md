...
- `/` — главная: studio form и catalog; глобальный player подключён выше маршрутов
- `/login` — auth panel
...
- `/admin` — staff-контроль последних загрузок, скрытия, восстановления и удаления треков
...
- `features/admin/AdminPanel.tsx` — staff-only review surface для последних загрузок и post-publication control
